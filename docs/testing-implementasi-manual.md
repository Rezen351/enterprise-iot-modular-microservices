...
| W9 | System-status notification | `GET /ws/system-status?token=` | notifikasi push (⚠️ planning: ditunda/belum) | [ ] | M4 |
...
| SEC4 | Kong rate-limit route lain | 300/min | 429 saat melampaui | [ ] | M2 |
...
| MSG9 | NATS `audit.log` | Core NATS | dipublish Auth/Module (belum di-consume) | [ ] | M2 |
| MSG10 | NATS `detection.result` | Pub/Sub | dipublish ML | [ ] | M3 |
| MSG11 | `alert.triggered`/`alert.resolved` | (belum ada Alert Svc) | [-] | — |
...
|---|-----|--------|------------|--------|--------|
| OBS1 | Prometheus scrape targets | `prometheus:9090/targets` | auth/module/analytics/wsgateway/kong/stream/ml UP | [ ] | M2 |
...
| D9 | System Notifications | NotificationContext | push via `/ws/system-status` (⚠️ belum) | [ ] | M4 |
...
|---------|------|-----------|-----------------|--------|
| Alert Service | 7 | 🔴 P1 | scaffold, sub `telemetry.ingest`, threshold (MariaDB+Redis), evaluasi, `alert.triggered`/`resolved`, `GET /alerts`, `PUT /alerts/:id/ack` | [ ] |
| Notification Service | 8 | 🟡 P2 | sub alert, FCM/Email/Telegram, queue Redis, log MariaDB | [ ] |
| Export / Data API | 11 | 🟢 P3 | `/export/v1/telemetry` (+aggregate via Analytics ✅), nodes, alerts, commands, audit, discover; rate-limit; caching | [ ] |
...
### 14a. ✅ Audit Service (sudah diimplementasikan — backend + infra)
...
| **M2** | Control + Stream + ML + WS + Analytics | C1–C27, S1–S16, V1–V19, W1–W5/W8, AN1–AN9, SEC2–8/10, MSG5/8, D4–D8, E2E2–5/7 | Fitur inti operasional lulus + kontrak firmware valid |
...
3. **`system-status` WS ditunda** — `NotificationContext` belum terhubung penuh. (W9, D9)
...
> **Catatan:** Service yang **belum diimplementasikan** (bagian 14: Alert, Notification, Audit, Export, OTA, dll) bukan bagian dari gate produksi fitur inti, tetapi harus dicatat sebagai *known limitations* pada saat deklarasi produksi. Sistem tetap bisa dinyatakan "siap produksi" untuk fitur yang sudah ada, dengan catatan limitation tersebut.
